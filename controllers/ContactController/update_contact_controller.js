    ...
    try{
        ...
        // after checking if the id's are not same then allow user to edit the contact but the name and phone number should match present in the DB
            if (contact.name === name) {
                contactExists = true;
                console.log(`Same name Contacts : ${contact}`);
                return res.render('edit_user',{contacts:contact,error:'Contact with this same name is already present in your account'});
                ...
                contactExists = true;
                console.log(`Same contacts number : ${contact}`);
                return res.render('edit_user',{contacts:contact,error:'Same contact number is already saved in your account'});
